...
CAMERA_WINDOW_POSITION = (50, 50)
EMBEDDED_IMAGE_MAX_WIDTH = 260
EMBEDDED_IMAGE_MAX_HEIGHT = 200
EMBEDDED_IMAGE_MARGIN = 12
...
def draw_embedded_image(frame, target_image, label):
    ...
    scale = min(
        EMBEDDED_IMAGE_MAX_WIDTH / image_width,
        EMBEDDED_IMAGE_MAX_HEIGHT / image_height,
        1.0,
    )
    ...
    label_height = 26
    box_width = resized_width + (EMBEDDED_IMAGE_MARGIN * 2)
    box_height = resized_height + label_height + (EMBEDDED_IMAGE_MARGIN * 2)

    box_x1 = frame_width - box_width - EMBEDDED_IMAGE_MARGIN
    box_y1 = frame_height - box_height - EMBEDDED_IMAGE_MARGIN
    box_x1 = max(EMBEDDED_IMAGE_MARGIN, box_x1)
    box_y1 = max(170, box_y1)
    box_x2 = min(frame_width - EMBEDDED_IMAGE_MARGIN, box_x1 + box_width)
    box_y2 = min(frame_height - EMBEDDED_IMAGE_MARGIN, box_y1 + box_height)

    overlay = frame.copy()
    cv2.rectangle(overlay, (box_x1, box_y1), (box_x2, box_y2), (0, 0, 0), -1)
    cv2.addWeighted(overlay, 0.45, frame, 0.55, 0, frame)

    image_x1 = box_x1 + EMBEDDED_IMAGE_MARGIN
    image_y1 = box_y1 + label_height + EMBEDDED_IMAGE_MARGIN
    image_x2 = image_x1 + resized_width
    image_y2 = image_y1 + resized_height

    frame[image_y1:image_y2, image_x1:image_x2] = resized_image
    cv2.rectangle(frame, (box_x1, box_y1), (box_x2, box_y2), (255, 255, 255), 2)
    cv2.putText(
        frame,
        f"Reference: {label}",
        (box_x1 + EMBEDDED_IMAGE_MARGIN, box_y1 + 18),
        ...
        while True:
            ...
            if current_time - last_recognition_time >= RECOGNITION_INTERVAL:
                ...
                if primary_scores:
                    ...
                    _, target_image = load_target_image(detected_gesture)
                    ...
                    if detected_gesture in GESTURE_IMAGE_MAP:
                        ...
                        score_overlay_lines = [
                            f"{model_name}: {model_scores[model_name]:.3f}"
                            for model_name in model_names
                        ]
            ...
            if score_overlay_lines:
                cv2.putText(
                    frame,
                    f"Model confidences for '{last_detected_gesture}':",
                    (10, 100),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.65,
                    (0, 255, 255),
                ...
                for index, score_line in enumerate(score_overlay_lines):
                    cv2.putText(
                        frame,
                        score_line,
                        (10, 130 + (index * 25)),
            ...
            if target_image is not None and last_detected_gesture is not None:
                draw_embedded_image(frame, target_image, last_detected_gesture)
